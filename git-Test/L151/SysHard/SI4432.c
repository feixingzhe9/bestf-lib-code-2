...
//	SPI_RW_Reg(W_REG | 0x33, 0x42);  //使用Header byte 0,1,2,3 为头码  /  同步字使用 3,2 
	...
	SPI_RW_Reg(W_REG | 0x79, 0x00);  //FrequencyHoppingChannelSelect
...
//			Fault_Upload(FAULT_2, SI4432_CRC_ERR, NULL); 2014.11.13 Unarty 屏蔽，此异常记录已失去意义	
		...
		if ((resVal1&0x02)) //packet received interrupt occured
		{
			...
			else
			{
				...
				if(data[25] == 0xB2 && data[27] == 0xBB \
					&& (Get_Sign433_MasterAddr()  == (*(u32 *)&data[18])))  //frame[25]=0xb2,frame[27]=0xbb
				{                                                      //frame[18]-frame[21]=测试主机地址
					TestRecRSSI[data[28]] = SI4432_RSSI;
				}
				if(Get_Sign433_MasterAddr() != (*(u32 *)&data[18]))
				{
					TestRecRSSI[80]++;  //默认为80包测试数据，第81包存放干扰包数量
				}
...
static u8 Check_433M_Busy(u8 level)
{
	const static u16 randWait_Tab[] = {5, 30, 200, 2000};
