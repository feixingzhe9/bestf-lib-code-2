...
const u8  KeyMapTab[17] = {0,  9, 1,14, 6,10, 2,12, 4, 7,11, 3,13, 5, 0, 0, 0};
...
/*测试flash*/
    ...
    if( ch != NONE_KEY)
    {
        ...
        OLED_ClearScreen();
        //OLED_ShowNumber(0,1,ch,2,0);
        OLED_ShowPicAt(4,0,64,64,PicTab[ch]);
        ShowLED_StartCycleLED(ch);
        StandbyCountReset();
        
        //ShakeMotor_Start();
        //Thread_Login(ONCEDELAY, 1, 500 , &ShakeMotor_Stop);
...
void Key_FirstHandle(u8 Key)
{
    ...
    {   
        OLED_ClearScreen();
        OLED_ShowPicAt(4,0,64,64,PicTab[Key]);
        ShowLED_StartCycleLED(Key);
