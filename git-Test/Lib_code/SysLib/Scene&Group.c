...
MsgResult_t Set_Scene_Attr(UnitPara_t *pData)
{
	...
	else if (((u8)(pData->data[0] + 1 + temp) > UnitCnt_Get())	//单元号不正确	（注意：清除指令为：0xFF, 所以加1后变成了零，符合条件）
			|| ((u8)(pData->data[5] + 1 + temp) > AREA_SCENEFLAG_NUM)//场景号不正确
			)
	{
...
MsgResult_t Set_Group_Attr(UnitPara_t *pData)
{
	...
	if (pData->len < 3)	//参数长度错误
	{
		return PARA_LEN_ERR;
			...
			|| ((u8)(pData->data[5] + 1 + temp) > AREA_GROUP_NUM)//场景号不正确
			)
	{
		return PARA_MEANING_ERR;
	}
