...
MsgResult_t Set_ResetAddr_Attr(UnitPara_t *pData)
{
    ...
    if((RE_DEVICE_ID_FLAG == (*(u16 *)(RE_DEVICE_ID_START_ADDR)))&&                 /**> 若地址已经重设过,校验当前设备地址与上位机发送的当前校验地址是否相同 **/                
       (RE_DEVICE_ID != 0xffffffff)&&
       (RE_DEVICE_ID != 0x00000000)&& 
       (RE_DEVICE_ID != 0x44444444)&& 
       (RE_DEVICE_ID != 0x88888888)&&  
       ...
       (RE_DEVICE_ID == ~RE_DEVICE_ID_ANTI)) 
    {
        if(RE_DEVICE_ID != (*(u32*)(&pData->data[4])))
        {
            return PARA_MEANING_ERR;                                                /**> 参数错误 **/          
        }
		else if(RE_DEVICE_ID == (*(u32*)(&pData->data[0])))							/**> 若重设地址已设置过，则直接返回成功 **/
		{
			return COMPLETE; 
		}
    }
	...
	*(u32*)(Tab_Tem + 10)   = FlashEncryptionFunc((*(u32*)(&pData->data[0])));
	/*重设地址功能数据格式为->重设地址标记(2bytes 0xaa55) + 重设地址(4bytes) + 重设地址反码(4bytes) */       
	*(u16*)(Tab_Tem + 0x00) = RE_DEVICE_ID_FLAG;
	*(u32*)(Tab_Tem + 0x02) = (*(u32*)(&pData->data[0]));
	*(u32*)(Tab_Tem + 0x06) =~(*(u32*)(&pData->data[0]));
	if(Program_Write_Ext(RE_DEVICE_ID_START_ADDR,14, (u8 *)(Tab_Tem)))
	{
